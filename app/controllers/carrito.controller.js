...
const confirmarCarritoCrearPedido = async (req, res) => {
    ...
    try {
        ...
        // 1) Bloqueamos SOLO el carrito (sin include)
        ...
        // 2) Obtenemos los items en una consulta separada (sin FOR UPDATE con LEFT JOIN)
        ...
        if (!items || items.length === 0) {
            ...
            return res
                .status(400)
                .json({ mensaje: "El carrito está vacío o no tiene items" });
        }

        // 3) Obtenemos las presentaciones
        const presentacionIds = [
            ...new Set(items.map((i) => i.id_presentacion_producto)),
        ];
        ...
        // 4) Calcular requerido por producto en unidad base y subtotal
        const requeridoPorProducto = {};
        let subtotalProductos = 0;

        for (const item of items) {
            const presentacion = mapaPresentaciones[item.id_presentacion_producto];
        ...
        // 5) Verificar stock disponible en la ubicación
            ...
            await DetallePedido.create(
                {
                    ...
                    cantidad_unidad_venta: Number(item.cantidad_unidad_venta),
                    cantidad_unidad_base: Number(cantidadBase),
        ...
        // 9) Actualizar inventario (reservar stock)
        for (const idProductoStr of Object.keys(requeridoPorProducto)) {
            const id_producto = parseInt(idProductoStr, 10);
        ...
        // 10) Marcar carrito como convertido y vaciar items
